...
class Info {
    ...
    private static updateSettings(screen: ScreenSize, media: MediaQuery) {
        ...
        const screenValue = '<span class="x-label">x</span>' + screen.x + '<span class="y-label">y</span>' + screen.y
        const mediaQueryValue = '<span class="media-icon"><i class="fas ' + media.icon + '"></i></span>'
            + media.label
